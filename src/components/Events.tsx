    ...
    >
      <section id="events">
        <div className="space-y-12 md:space-y-16">
          ...
          <ul
            className={`grid mx-auto ${eventWrapperClass} gap-8 md:gap-10`}
          >
            {events.map((event) => (
              <EventCard
                key={event._key}
                ...
                <div className="overflow-auto max-w-5xl mx-auto">
                  <div className="flex-1 space-y-5 sm:space-y-8 lg:space-y-12">
                    <div className="space-y-2">
                      ...
                      {selectedEvent.info.subheadline && (
                        <BlockContent
                          value={selectedEvent.info.subheadline}
                        />
                      )}
                    </div>
  ...
  return (
    <ul className="space-y-6">
      {details.map((detail, idx) => (
        <div key={detail._key} className="border-4 cursor-pointer hover:bg-black/10">
          <button
            type="button"
          ...
          <div
            ...
            role="region"
            aria-labelledby={`accordion-header-${detail._key}`}
...
function EventCard({
  ...
  return (
    <div
      className="relative border-4 px-4 py-6 bg-white flex flex-col gap-6 sm:gap-8 sm:px-6 sm:py-10"
    >
      {event.tag && (
        <p
      ...
      <div className="space-y-1">
        {event.location && (
          <h6 className={typography.h6}>{event.location}</h6>
        )}
